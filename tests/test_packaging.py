...
def test_version_is_a_single_source_of_truth() -> None:
    ...
    try:
        installed = version("coho-sdk")
    except PackageNotFoundError:  # pragma: no cover - only when running from a bare checkout
        pytest.skip("coho-sdk is not installed in this environment")
    assert installed == coho_sdk.__version__


def test_the_package_is_typed() -> None:
    """`py.typed` must ship, or every consumer's type checker silently ignores us."""
    assert (Path(coho_sdk.__file__).parent / "py.typed").is_file()


def test_public_names_are_importable() -> None:
    for name in coho_sdk.__all__:
        assert hasattr(coho_sdk, name), f"__all__ names {name}, which does not exist"


def test_testing_helpers_are_not_imported_by_default() -> None:
    """`coho_sdk.testing` needs pytest-httpserver, which is an extra; importing the
    package must not drag it in."""
    import sys

    assert "coho_sdk" in sys.modules
    source = Path(coho_sdk.__file__).read_text()
